...
user = "root"
password = "gass_1985"
mysql_connection = "mysql -h 192.168.0.21 -u%s -p%s  --local-infile=1 eggNOG "%(user,password)
mysql_build = "mysql -h 192.168.0.21 -u%s -p%s  --local-infile=1 "%(user,password)
connection_string = 'mysql://%s:%s@192.168.0.21/eggNOG'%(user,password)    
...
if __name__ == '__main__':
    ...
    r = redis.Redis(host='192.168.0.21',port=6379,db=int(db_number))
    r.flushdb()      
