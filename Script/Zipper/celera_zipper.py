...
from get_reference_network import Get_reference_network
...
def get_para(   ):
    ...
    parser.add_option("-k", "--REF", action="store",
                          dest="reference",
                          help="reference_graph cele_blat.py generated")   
...
if __name__=='__main__':
    ...
    BEST = fasta_check(  open( options.best ,'rU'  )  )
    terminal = options.terminal
    reference = options.reference
    ####################
    #获得reference 图
    refer3_network,refer5_network= Get_reference_network( open( reference,'rU'  )   ) 
    ...
    for each_unitig in sorted(all_unitig ,key = lambda x: int(x)):
        ...
        TRIM_CACHE_READS.write( '>head\n'+all_best_reads_need[ cache[0][:-1] ] )
        TRIM_CACHE_READS.write( '>tail\n'+all_best_reads_need[ cache[-1][:-1] ] )
    ...
    for key in unitig_graph_3:
        if key in  overlap_3_network and overlap_3_network[key]!= unitig_graph_3[key] and  unitig_graph_3[key]!= refer3_network[key] :

            all_different[  key[:-1]  ] = ''
            different_3[ key  ] = ''
    for key in unitig_graph_5:		
        if key in overlap_5_network   and overlap_5_network[key]!= unitig_graph_5[key] and  unitig_graph_5[key]!= refer5_network[key]:
    ...
    for utgs in sorted(  every_contig_detail ,key=lambda x: int(x) ):
        ...
        for each_data in  every_contig_detail[ utgs]:
            if each_data[:-1] in all_different:
                i=1
                CACHE_READS.write( '>%s\n%s'%( each_data[:-1] , all_best_reads_need[ each_data[:-1] ])  )
        if i==1:
            ...
            for node,[ start,end  ] in nucmer_out:
                ...
                if ['5'] == status and not  unitig_graph_5[ node ]: 
                    ...
                    CACHE_READS2.write( '>%s\n%s'%( unitig_graph_5[ node  ].keys()[0][:-1], all_best_reads_need[ unitig_graph_5[ node  ].keys()[0][:-1] ])  )
                    #CACHE_READS2.close()
                ...
                if status==['3'] and node not in unitig_graph_3: 
                    ...
                    CACHE_READS2 = open( 'cache2_reads.fasta','w' )
                    CACHE_READS2.write( '>%s\n%s'%( unitig_graph_3[ node  ].keys()[0][:-1], all_best_reads_need[ unitig_graph_3[ node  ].keys()[0][:-1] ])  )
                    os.system( 'nucmer --maxmatch cache_unitig.fasta cache2_reads.fasta -p cache2 >/dev/null 2>&1'     )
                    nucmer_out = subprocess.check_output(  shlex.split('show-coords -cT  cache2.delta' )  )
                    nucmer_out = GET_nucmer_best(   nucmer_out )
